...
**Refero Style:** https://styles.refero.design/style/da0bfca3-df1d-49d9-ae25-61d8f348426f
...
| --- | --- | --- | --- |
| Brand Blue | `#145fe4` | `--color-brand-blue` | Primary CTAs, active links, focus indicators â the single point of interactive confirmation. |
| Hero Fade | `#145fe4` | `--color-hero-fade` | Hero background â creates a deep, atmospheric transition from dark space to brand blue. |
...
| Warning Tan | `#cbb88` | `--color-warning-tan` | Top announcement banner background â a muted, temporary accent for site-wide notices. |
...
### Feature Cards â Discover, Secure, Audit
...
Text in Brand Blue (#145fe4), typically followed by a 'â' character. No underline by default, likely added on hover.
...
2.  **Feature Card Grid:** "On a white page background, generate a 3-column grid of cards. Each card must have: `backgroundColor: #ffffff`, `padding: 24px`, `borderRadius: 16px`, and a subtle `box-shadow: 0 4px 12px 0 rgba(0,0,0,0.05)`. The card headline should be `agileSans` 32px weight 400 color `#000000`, body text 16px weight 400 color `#303136`, and a link with text `Learn more â` in 16px weight 500 color `#145fe4`."
...
:root {
  ...
  --font-agilesans: 'agileSans', Inter, Manrope, ui-sans-serif, system-ui, sans-serif;
