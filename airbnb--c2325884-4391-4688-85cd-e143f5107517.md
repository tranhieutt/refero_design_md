...
> Vacation photos pinned to a white corkboard â bright photography contained in rounded frames against a near-white surface, with a single coral pin holding everything together.

**Theme:** light
**Source:** https://www.airbnb.com
**Refero Style:** https://styles.refero.design/style/c2325884-4391-4688-85cd-e143f5107517

Airbnb's interface is a warm, airy marketplace built on near-white surfaces and a single coral-red brand heartbeat. The #f7f7f7 canvas with #ffffff card surfaces creates a barely-there depth â paper stacked on paper â where photography does all the visual heavy lifting. The custom Airbnb Cereal VF variable font runs at weights 400â700 across a compact, tight scale with negative letter-spacing at larger sizes, giving the UI a quiet confidence without needing decorative elements. The singular #ff385c coral-red appears exclusively on brand touchpoints (logo, active states, the search button), making it feel like a signature rather than a system color. Card images bleed edge-to-edge with 20px rounded corners â the only generous radius in an otherwise compact, information-dense layout.

## Tokens - Colors

| Name | Value | Token | Role |
| --- | --- | --- | --- |
| Rausch Coral | `#ff385c` | `--color-rausch-coral` | Brand logo fill, active nav underline, search button icon background, carousel dot active state â the single chromatic signature that makes the interface recognizable |
| Rausch Deep | `#e00b41` | `--color-rausch-deep` | Hover state darkening of Rausch Coral on interactive brand elements |
| Carbon | `#222222` | `--color-carbon` | Primary text, headings, borders, icon strokes â the dominant neutral forming almost all typographic content |
...
### Airbnb Cereal VF - The sole typeface for every text element across the UI. Weight 400 handles body and metadata; 500 lifts labels and secondary headings; 600 marks section titles; 700 for price display and primary emphasis. The custom variable font brings tight optical metrics at display sizes that generic fonts cannot replicate â negative letter-spacing at -0.02em pulls large numerals together. The 'salt' feature set activates alternate letterforms that distinguish it from system sans-serifs. - `--font-airbnb-cereal-vf`
- **Substitute:** Inter Variable
- **Weights:** 400, 500, 600, 700
- **Sizes:** 8px, 11px, 12px, 13px, 14px, 16px, 20px, 21px, 22px, 28px
- **Line height:** 1.18â1.43
- **Letter spacing:** -0.02em at large sizes (20px+), -0.009em at mid sizes (14â16px), +0.04em at small caps/label contexts (11â12px)
...
| heading | 22px | 1.23 | - | `--text-heading` |
| display | 28px | 1.18 | - | `--text-display` |
...
### Circular Icon Button â Default
...
### Rounded Pill Button â Secondary
...
### Do
- Use #ff385c exclusively for the brand logo, the search trigger button background, and active/selected state indicators â never for decorative fills or text blocks.
...
- Set all typography in Airbnb Cereal VF (substitute: Inter Variable) with font-feature-settings: 'salt'; apply letter-spacing -0.02em at 20px+ sizes and +0.04em for small-caps labels at 11â12px.
- Layer shadows as the three-value stack â rgba(0,0,0,0.02) 0px 0px 0px 1px, rgba(0,0,0,0.04) 0px 2px 6px 0px, rgba(0,0,0,0.1) 0px 4px 8px 0px â on elevated containers like the search bar and modals only; listing cards use no shadow.
- Maintain the surface hierarchy strictly: page canvas #f7f7f7 â card/header surface #ffffff â disabled/placeholder fills #dddddd â skeleton loaders #b0b0b0.
...
### Don't
- Never use #ff385c for body text, headings, or decorative strokes â its appearance outside logo/search-button/active-states breaks the signature scarcity of the brand color.
- Don't add heavy drop shadows to listing cards â cards use no shadow; the #dddddd placeholder color and 20px radius carry the visual weight without elevation.
...
- Don't use border-radius values other than 4px, 8px, 14px, 20px, 32px, or 50% â mixing arbitrary radii (e.g. 6px, 12px, 24px) disrupts the deliberate radius vocabulary.
- Never place text directly on card photography without a scrim or badge surface â overlay content uses #ffffff or semi-transparent white containers, not raw text on image.
- Don't use the blue gradient (rgb(45,60,91) â rgb(128,157,192)) as a UI surface â it is specific to the hero CTA card animation and not part of the repeatable component palette.
- Never omit the 'salt' font feature setting on Airbnb Cereal VF â without it, the alternate letterforms revert to generic shapes that undermine brand distinctiveness.

## Surfaces

| Level | Name | Value | Purpose |
| --- | --- | --- | --- |
| 1 | Canvas | `#f7f7f7` | Page background and footer fill â the paper beneath everything |
...
3. **Section Header**: Text 22px Airbnb Cereal VF weight 600 #222222, letter-spacing -0.2px, followed immediately by a â  arrow icon in #222222. Bottom margin 12px before the card carousel. Subtext (optional) 14px weight 400 #6a6a6a directly below heading.
...
Transitions run at 0.25s ease for most interactive state changes (hover backgrounds, shadow lifts), with 0.1s for immediate snap feedback (button presses). The homepage hero card stack uses a custom spring curve â linear(0 0%, 0.238367 8.33%... 1 100%) â for the stacked card fan animation, giving it a physical bounce that distinguishes it from the otherwise utilitarian transition vocabulary. Transform is the primary transitioned property (141 instances), reinforcing that the design moves things rather than fades them. The backdrop-filter: blur(32px) is used on overlay surfaces (e.g. sticky compact search on scroll).
...
- **Etsy** - Same surface hierarchy (off-white canvas â white card â no card shadow) with photography as the sole source of visual richness and a single warm-hued brand accent
...
:root {
  ...
  --color-fog: #f7f7f7;
  --color-cloud: #ffffff;
  --font-airbnb-cereal-vf: 'Airbnb Cereal VF', Inter Variable, ui-sans-serif, system-ui, sans-serif;
  --text-caption: 11px;
