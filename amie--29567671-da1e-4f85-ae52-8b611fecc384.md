...
> Sunlit productivity dashboard â a workspace where everything is white, flat, and purposeful, with one electric-blue switch that turns things on.

**Theme:** light
**Source:** https://amie.so
**Refero Style:** https://styles.refero.design/style/29567671-da1e-4f85-ae52-8b611fecc384

Amie feels like a well-lit productivity workspace â bright white surfaces, almost clinical in their restraint, punctuated by a single electric sky-blue (#11a8ff) that fires only when action is required. The page is overwhelmingly neutral â 98% achromatic â making every chromatic moment (the amber highlight on 'without a bot', the green calendar icon, the violet tag) feel intentional rather than decorative. Inter at tight negative letter-spacing (-0.025em at display sizes) condenses headlines into confident blocks without weight extremes â weight 700 at 56px reads assertive but not aggressive. Cards use hairline 1px borders via layered near-invisible shadows rather than visible strokes, giving product UI previews the appearance of floating slightly off the page. The ghost 'Request a demo' button beside the filled CTA creates a paired choice architecture that recurs throughout the page.
...
| Ash Border | `#cdcdcd` | `--color-ash-border` | All UI borders â inputs, cards, dividers, icon rings â at 1px solid |
...
| Sky Action | `#11a8ff` | `--color-sky-action` | CTA buttons ('Get started', 'Start free trial'), active nav highlight, brand accent â the sole chromatic action color on an otherwise monochrome page |
| Sky Pale | `#cfeeff` | `--color-sky-pale` | Highlight backgrounds behind key phrases ('in just 7 days'), decorative gradient start â sky tint without saturation |
...
| Amber Highlight | `#fbefaf` | `--color-amber-highlight` | Text highlight background behind 'without a bot' in hero headline â single warm note in a cool palette |

## Tokens - Typography

### Inter - Single typeface for all contexts â headlines to captions. Weight 700 at 56px for hero display, 600 at 40px for section headings, 400â500 for body and UI. The -0.025em tracking at large sizes (â-1.4px at 56px, â-0.48px at 40px) compresses headlines into tight confident blocks. No decorative or serif counterpart â mono-typographic system. - `--font-inter`
- **Substitute:** Inter (Google Fonts)
- **Weights:** 400, 500, 550, 600, 700
- **Sizes:** 12px, 13px, 14px, 16px, 20px, 40px, 56px
- **Line height:** 1.00â1.75 (display: 1.00â1.14, body: 1.50â1.75)
- **Letter spacing:** -1.40px at 56px (-0.025em), -0.48px at 40px (-0.012em), normal at 16px and below
- **OpenType features:** `normal`
- **Role:** Single typeface for all contexts â headlines to captions. Weight 700 at 56px for hero display, 600 at 40px for section headings, 400â500 for body and UI. The -0.025em tracking at large sizes (â-1.4px at 56px, â-0.48px at 40px) compresses headlines into tight confident blocks. No decorative or serif counterpart â mono-typographic system.

### Inter var - Inter var â detected in extracted data but not described by AI - `--font-inter-var`
- **Weights:** 400
- **Sizes:** 16px
- **Line height:** 1.75
- **Role:** Inter var â detected in extracted data but not described by AI
...
### Primary CTA Button
**Role:** Main conversion actions â 'Get started', 'Start free trial'
...
Transparent background, text #000000 or #5c5c5c, no border radius (0px), 20px padding all sides. Inter weight 400â500, 14px. Appears in nav bar and inline contexts.
...
Background #fafafa, 12px radius, same 3-layer shadow as white card variant. Padding 0 â inner content uses its own spacing.

### Highlight Chip
**Role:** Inline text emphasis tags like 'Within 47 seconds:' label
Background #ebebeb, 16px radius, 8px/16px padding, no shadow. Inter weight 600, 14â16px, text #000000. Used to isolate a key statistic or phrase visually.

### Category Tag (Pill)
**Role:** Feature category labels â 'Meeting Notes', 'Shareable Pages'
...
Logos rendered at grayscale(1), horizontally scrolling via 'scrollX' animation (70s linear). No card container â logos sit directly on white page background. Inter caption 12px #5c5c5c for label text.

## Do's and Don'ts

### Do
- Use #11a8ff exclusively for filled CTA buttons â no other UI element gets a chromatic fill color
...
- Highlight key statistics or phrases inline with #fbefaf background at the span level â apply only once per hero to preserve impact
- Maintain grayscale(1) filter on partner/social proof logos and product screenshots in secondary contexts
- Keep category accent colors (#01ca45, #a050ff) to border-only usage on tags â never fill a UI surface with them

### Don't
- Never add a second chromatic action color â #11a8ff is the only CTA fill; competing colors collapse the hierarchy
- Do not use weight below 400 for body text or above 700 for any text â the weight range 400â700 is the entire system
- Avoid explicit border declarations for cards â the shadow-as-border technique (0.06 opacity ring) is what keeps surfaces integrated, not separated
- Never use #a0a0a0 or #5c5c5c for headings â these are strictly secondary body and label colors
- Do not center-align body paragraphs â left-aligned text with 16px subheading baseline is the consistent content pattern
- Avoid gradients in UI components â the sky gradient (#cfeeff â #41baff) is decorative-only behind highlighted phrases, not a button or card treatment
- Do not apply backdrop-filter blur outside the navigation bar â it appears only on the sticky nav at scroll
...
1. **Sky Wash**: linear-gradient(to right top, #cfeeff, #41baff) â used as background tint behind highlighted phrases or feature sections. Never on buttons or cards.
2. **Blue Depth**: linear-gradient(to right top, #11a8ff, #41baff) â denser sky gradient for branded decorative blocks. Both share the same directional angle (top-right). Do not introduce gradients in other hue families.
