...
> Gallery wall at natural light â enormous type casts shadows on a white surface, color enters only as product.

**Theme:** light
**Source:** https://www.apple.com/macbook-neo
**Refero Style:** https://styles.refero.design/style/c9cabb96-32fa-4896-837a-f2497ce1c856

Apple's MacBook Neo product page radiates cool luminosity â a gallery-white canvas where enormous weight-700 headlines at 80-96px dominate above feather-light body copy, creating a tension between mass and air. The page background stays #f5f5f7, one step off pure white, giving cards and content wells a surface-lift effect without any shadows. Negative letter-spacing tightens progressively with size â display headlines track at -0.022em while body text breathes at -0.003em, a signature move that makes large type feel chiseled rather than loose. The single interactive accent (#0071e3 CTA blue) appears only on the 'Buy' pill button and nav links, rationed so every appearance reads as an instruction to act. Product gradients â dark-to-citrus-green, dark-to-blue, dark-to-violet â serve as full-bleed theatrical backdrops for each color finish showcase, never decorating UI chrome.

## Tokens - Colors

| Name | Value | Token | Role |
| --- | --- | --- | --- |
| Ink | `#1d1d1f` | `--color-ink` | Primary text, headings, nav labels, icon fills â near-black with just enough warmth to avoid harshness on white |
...
| Azure | `#0071e3` | `--color-azure` | Primary CTA button fill, active nav 'Buy' button â the sole permission-to-act color on the entire page |
| Cobalt Link | `#0066cc` | `--color-cobalt-link` | Inline text links, navigation anchor colors â slightly darker than Azure to distinguish interactive text from interactive buttons |
| Citrus Gradient | `#dddc8c` | `--color-citrus-gradient` | MacBook Neo Citrus finish showcase backdrop â dark-to-citrus theatrical product gradient |
| Indigo Gradient | `#2535e2` | `--color-indigo-gradient` | MacBook Neo Indigo finish showcase backdrop â dark-to-deep-blue theatrical product gradient |
| Blush Gradient | `#cc29bc` | `--color-blush-gradient` | MacBook Neo Blush finish showcase backdrop â dark-to-magenta-violet theatrical product gradient |
| Citrus Finish | `#dddc8c` | `--color-citrus-finish` | Product color swatch â Citrus finish selector chip |
| Blush Finish | `#e8d0d0` | `--color-blush-finish` | Product color swatch â Blush finish selector chip |
| Indigo Finish | `#596680` | `--color-indigo-finish` | Product color swatch â Indigo finish selector chip |
| Caution | `#b64400` | `--color-caution` | Badge warning text, price asterisk callouts â deep amber-orange for inline semantic alerts |

## Tokens - Typography

### SF Pro Display - All marketing headlines, product name lockups, section titles. Weight 700 at 80-96px is the page's defining visual move â these headlines are 2-4 words at near-poster scale, taking up half the viewport before any product image appears. Negative tracking at display sizes (-0.022em at largest) tightens letterforms into a single typographic mass. - `--font-sf-pro-display`
- **Substitute:** Inter, system-ui
- **Weights:** 600, 700
- **Sizes:** 21px, 24px, 28px, 32px, 40px, 56px, 80px, 96px
- **Line height:** 1.04â1.20
...
- **Role:** All marketing headlines, product name lockups, section titles. Weight 700 at 80-96px is the page's defining visual move â these headlines are 2-4 words at near-poster scale, taking up half the viewport before any product image appears. Negative tracking at display sizes (-0.022em at largest) tightens letterforms into a single typographic mass.
...
backgroundColor #0071e3, color #ffffff, borderRadius 999px, paddingTop 8px, paddingBottom 8px, paddingLeft 16px, paddingRight 16px, font SF Pro Text 17px weight 400. Appears in global nav and anchored product nav. The only #0071e3 element on the page â isolated blue against all-neutral surroundings makes it impossible to miss without competing colors.

### Ghost Text Button
**Role:** Section 'Learn more' links styled as buttons
backgroundColor transparent, color #1d1d1f, borderRadius 0px, no padding. Rendered as plain weighted text. Used for secondary actions inside feature cards â avoids visual noise next to the single primary CTA.
...
backgroundColor #000000, color #ffffff, borderRadius 999px, paddingLeft 16px, paddingRight 16px. Used when the backdrop is a product color gradient â switches from #0071e3 to #000000 to remain legible against vivid gradient backgrounds.

### White Feature Card
**Role:** Primary content card for feature showcases
backgroundColor #ffffff, borderRadius 28px, boxShadow none, padding 28px. Lifts off the #f5f5f7 canvas by 1 surface level. The 28px radius is signature Apple â rounded enough to feel premium, not so round it reads as a chip or badge.

### Fog Feature Card
**Role:** Alternate feature card on white backgrounds
backgroundColor #f5f5f7, borderRadius 28px, boxShadow none, padding 28px. Used when the parent section is #ffffff â reverses the surface/card relationship so the card reads as recessed rather than elevated.

### Dark Feature Card
**Role:** High-contrast product detail card
backgroundColor #000000, borderRadius 28px, boxShadow none. Text color #ffffff. Used as a single accent card within a light card grid â maximum contrast without color.
...
backgroundColor transparent, color #1d1d1f, borderRadius 0px, font SF Pro Text 12px weight 600. Appears as a plain text label, not a pill â anti-convention for a badge, reads as an editorial stamp rather than a UI chip.
...
### Do
- Use 28px border-radius for all feature cards (#ffffff and #f5f5f7 backgrounds) â this exact value is the page's geometric signature.
- Reserve #0071e3 exclusively for the primary 'Buy' CTA button. No other UI element uses this blue â its scarcity makes it the only color that reads as an imperative.
...
- Use #f5f5f7 as page canvas and #ffffff as card surface. Never reverse this â cards must always be lighter than their parent background.
...
- Use rgba(210,210,215,0.64) with backdrop-filter blur(20px) for any temporary overlay controls (country selectors, segmented pickers) â this is the frosted-glass layer distinct from all opaque surfaces.
...
### Don't
- Do not add box-shadow to any card or container. The entire elevation system is color-value-only â shadows break the flat-lit gallery effect.
- Do not use #0066cc (Cobalt Link) for button backgrounds. It is an inline text link color only â using it on filled buttons conflicts with the #0071e3 CTA hierarchy.
- Do not use font weight 300 for headlines below 40px â below that threshold, weight 300 loses definition against the white/fog background.
...
- Do not use the product finish colors (#dddc8c Citrus, #e8d0d0 Blush, #596680 Indigo) as semantic or UI accent colors â they exist only as product identity swatches and gradient source colors.
- Do not use positive letter-spacing on SF Pro Display at sizes above 28px. The +0.007emâ+0.011em values from the data appear only at small display sizes (21px); at headline scale positive tracking is tonally inconsistent.
...
| --- | --- | --- | --- |
| 0 | Canvas | `#f5f5f7` | Page body background â one step off white, creates implicit depth against pure-white cards |
| 1 | Card | `#ffffff` | Feature cards, info panels, modal containers â lifts off canvas without shadow, pure separation by value |
| 2 | Recessed | `#f5f5f7` | Inset content areas within white cards â same as canvas, creates nested depth by repetition |
| 3 | Frosted Control | `#d2d2d7` | Pill selector backgrounds (country dropdown, segmented controls) â rgba(210,210,215,0.64) with backdrop blur |
...
## Imagery
Product photography dominates: the MacBook is always the sole subject, photographed on pure white or against the Fog (#f5f5f7) surface â zero lifestyle context, no hands (except the hero hand hold), no desk environment. The hero image is the laptop balanced on a single palm from below-frame, shot at a near-overhead angle with the vivid Citrus finish as the sole color note in an otherwise achromatic composition. Color finish showcases use full-bleed theatrical gradients (dark-to-citrus, dark-to-indigo, dark-to-blush) as the backdrop, with the open laptop emerging from the gradient â product-as-light-source. No illustration. Icons are SF Symbols style: mono, filled/outlined hybrid, 1.5-2px apparent stroke, #1d1d1f on light backgrounds, #ffffff on dark cards. The page is image-heavy relative to text â each major section is anchored by a product image occupying 50-80% of viewport height, with text floating above or below in compact lockups.

## Layout
Max-width ~1200px centered on a #f5f5f7 canvas. Hero is full-viewport centered headline stack (product name at 24px, hero headline at 80-96px, price subtext at 17px, single Buy pill) with product image breaking below the text fold â classic Apple centered-stack hero. Sub-nav becomes sticky at ~52px height after the hero. Section rhythm: alternating white (#ffffff full-width bands) and fog (#f5f5f7) bands with no explicit dividers â the color shift alone creates section breaks. Feature sections use 2-column split (text left at ~40% width, product image right at ~60%) or full-bleed product gradient cards. Highlight carousel is a single-column centered card with horizontal scroll and pagination dots. Specs section uses a tight left-label / right-value two-column table pattern at body-sm size. Feature detail cards stack in a 2Ã2 grid at 28px radius, each card self-contained. Navigation: translucent sticky global nav at top (12px text, ~44px tall) plus sticky product sub-nav below it (14px text, ~52px tall) â two navigation layers are always visible, keeping section context persistent during long scroll.

## Agent Prompt Guide

QUICK COLOR REFERENCE:
â¢ Page background: #f5f5f7
â¢ Card surface: #ffffff
â¢ Primary text / headlines: #1d1d1f
â¢ Secondary text: #707070
â¢ CTA button fill: #0071e3
â¢ Inline links: #0066cc
â¢ Border / divider: #e8e8ed
...
Primary duration: 0.344s (page transitions, card reveals). Quick feedback: 0.1s (hover states, button presses). Scroll-driven animations: 0.32s ease. Default easing: ease (CSS keyword) for 97% of transitions. background-color and color transitions are the most common animated properties â hover states shift between neutral values. opacity transitions power section entrance reveals. transform transitions handle product image parallax and carousel movement. The overall motion personality is 'expressive' but anchored by slow-ease timing that feels gravity-weighted rather than springy.

## Product Finish Color System

Four hardware finishes map to CSS tokens and gradient backdrops:
â¢ Silver: swatch #e3e4e5 (--finish-silver), no gradient showcase
â¢ Blush: swatch #e8d0d0 (--finish-blush), gradient from #1d1d1f â #f3c4f6 â #f500b4 â #cc29bc
â¢ Citrus: swatch #dddc8c (--finish-citrus), gradient from #1d1d1f â #dfe74f â #5e9c2a â #0a8619
â¢ Indigo: swatch #596680 (--finish-indigo), gradient from #1d1d1f â #a8d3fb â #0012f9 â #2535e2
Each swatch chip: 28px diameter, borderRadius 999px. Selected state: 3px solid border in #1d1d1f (on light) or #ffffff (on dark). Gradient showcases are full-bleed 184Â° linear gradients used exclusively as section backdrops, never UI fills.
...
:root {
  ...
  --font-sf-pro-display: 'SF Pro Display', Inter, system-ui, ui-sans-serif, system-ui, sans-serif;
  --font-sf-pro-text: 'SF Pro Text', Inter, system-ui, ui-sans-serif, system-ui, sans-serif;
  --text-caption: 12px;
