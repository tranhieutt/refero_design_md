...
| Asana Violet | `#222875` | `--color-asana-violet` | Hero product UI, informational badges, branded accent blocks â deep indigo-navy grounds AI and premium feature messaging |
| Coral Blush | `#ffeaec` | `--color-coral-blush` | Active filter chip background, soft badge backgrounds, warm section fills â coral warmth signals interaction state without aggression |
| Deep Coral | `#690031` | `--color-deep-coral` | Coral-family high-contrast text on Coral Blush backgrounds, brand icon accents |
| Coral Ember | `#ff584a` | `--color-coral-ember` | Icon fills in use-case cards, logo mark accent â vivid red-coral appears only in small illustration contexts |
...
### Ghost - Display headlines only â the hero statement and major section titles. Weight 500 at 60-72px with tight -0.007em tracking compresses letter forms into dense editorial blocks. No other font on the site operates at this scale, making Ghost the unambiguous voice of authority. - `--font-ghost`
...
- **Role:** Display headlines only â the hero statement and major section titles. Weight 500 at 60-72px with tight -0.007em tracking compresses letter forms into dense editorial blocks. No other font on the site operates at this scale, making Ghost the unambiguous voice of authority.

### TWK Lausanne - Everything except hero display text: nav labels (weight 400, 14px), body copy (weight 300â400, 16px), subheadings (weight 500, 20-30px), section headings (weight 500, 24-36px). Weight 300 for body imparts an open, non-urgent reading texture. Weight 500 for UI labels and mid-hierarchy headings. Positive letter-spacing at small sizes (+0.025â0.04em) prevents optical crowding in captions and badges. - `--font-twk-lausanne`
- **Substitute:** Lausanne (Weltkern), Inter, DM Sans
- **Weights:** 300, 400, 500
- **Sizes:** 11px, 12px, 13px, 14px, 16px, 20px, 22px, 24px, 30px, 36px
- **Line height:** 1.20 â 2.57 depending on size (large sizes ~1.20, body ~1.50, captions ~1.75)
- **Letter spacing:** Ranges from -0.01em at large sizes to +0.04em at caption/label sizes
- **Role:** Everything except hero display text: nav labels (weight 400, 14px), body copy (weight 300â400, 16px), subheadings (weight 500, 20-30px), section headings (weight 500, 24-36px). Weight 300 for body imparts an open, non-urgent reading texture. Weight 500 for UI labels and mid-hierarchy headings. Positive letter-spacing at small sizes (+0.025â0.04em) prevents optical crowding in captions and badges.

### Type Scale
| Role | Size | Line Height | Letter Spacing | Token |
| --- | --- | --- | --- | --- |
| caption | 11px | 1.75 | - | `--text-caption` |
...
Background #0d0d0d, text #ffffff, border-radius 100px, padding 16px 32px. TWK Lausanne weight 500 at 16px. No border. The extreme pill radius against square card containers is the site's primary tension â the softness of the button against the geometry of surrounding content.

### Secondary Outline Button (Pill)
**Role:** Secondary actions: 'View demo', alternate CTAs alongside primary
Background #f3f3f3, text #0d0d0d, border 1px solid #0d0d0d, border-radius 146px, padding 16px 32px. Matches primary pill shape but inverted surface â light fill with a defined border. Near-identical radius to primary (146px vs 100px) keeps the two visually paired.

### Ghost Nav Button
**Role:** Navigation dropdown triggers and tertiary utility actions
Background transparent, text #6e6e6, border-color #6e6e6 (1px), border-radius 3px, padding 0 12px. No vertical padding. TWK Lausanne weight 400 at 14px. The 3px radius is the sharpest shape on the site â nav triggers are deliberately not pill-shaped to distinguish them from CTAs.

### Filter Chip (Active)
**Role:** Active category filter in tabbed product sections: 'Marketing', 'Operations', etc.
Background #ffeaec, text #690031, border-radius 999px, padding 8-12px horizontal. Active state surfaces the coral brand family â warm against the white section background. Inactive sibling chips use #f3f3f3 background with #0d0d0d text.

### Product UI Screenshot Card
**Role:** Hero and section feature showcases â framed product app screenshots
...
White background, no border, no shadow. Logos displayed in grayscale at medium opacity, horizontally spaced at 32-48px columnGap. Stat text ('85% of Fortune 100') at TWK Lausanne weight 500, 30px, #0d0d0d. Logos: Amazon, Accenture, Johnson & Johnson, Dell, Merck â all rendered in flat black/gray.
...
### Do
- Use Ghost font at 60-72px with letter-spacing -0.0070em exclusively for top-level display headlines â no other element should compete at this scale
- Apply 100px+ border-radius on all CTA buttons (primary and secondary) and filter chips â pill shapes are the only soft element in an otherwise angular component set
- Reserve #222875 (Asana Violet) for product UI frames and informational containers â never use it as a button fill or section background
- Pair Coral Blush (#ffeaec) backgrounds exclusively with Deep Coral (#690031) text â they are a locked semantic pair; do not substitute either half
...
- Use TWK Lausanne weight 300 for all body copy to preserve the open, non-urgent reading texture â weight 400 is for UI labels and nav, weight 500 for subheadings and CTAs
- Set card borders to 1px solid #e7e7e7 with 10-16px radius â no box shadows on content cards; elevation is conveyed through color contrast alone

### Don't
- Never use Ghost font below 60px â TWK Lausanne at weight 500 handles all sub-display heading sizes
- Never apply the pill button radius (100px+) to cards, inputs, or modal containers â the shape language is exclusive to interactive action elements
- Never fill a full page section with #222875 or any saturated color â the violet and coral appear only in contained UI frames or chip-sized elements
- Never use positive letter-spacing on headings â letter-spacing is negative or zero for all sizes 20px and above
- Never mix the coral and violet accent families in the same component â they operate as separate semantic threads
- Never add box-shadow elevation to feature cards â borders alone define card boundaries; shadows break the flat, print-like surface quality
- Never set body copy weight above 400 â weight 500 is reserved for labels, CTAs, and subheading roles only

## Imagery
Two visual registers coexist. Product UI screenshots dominate: flat, pixel-accurate app window captures dropped inside rounded violet frames, shown at large scale (full-width hero) and medium scale (section features). These are isolated, never lifestyle-contextualised â the interface IS the subject. The second register is a set of outline-only line illustrations used in use-case cards: single-color coral stroke illustrations on white (a campaign chart, an easel, a rocket) with minimal fill, geometric-organic hybrid style, approximately 1.5px apparent stroke weight. Icons across the UI are monochrome outlines, matching the TWK Lausanne light weight in visual mass. No photography anywhere on the visible page. The density balance is text-dominant with product UI serving as the single large visual anchor per section.

## Layout
Max-width approximately 1200px centered on all content, but product UI screenshot cards bleed to wider containers. Hero is a centered-headline pattern: Ghost headline centered, subhead centered at ~560px max-width, two pill buttons in a row, then a full-width product UI screenshot card below. Sections below alternate between: full-width white logo bar (no containers), left-text + tabbed-filter + image right (2-column split), and horizontally scrolling card rails (4 cards visible, 10px gap, overflow hinted). Navigation is a fixed top bar at 56px with left logo, center nav, right utility links. Vertical section rhythm is generous â 80-120px between sections â giving each content cluster breathing room. The feature card carousel section uses explicit prev/next circular icon buttons for navigation control.
...
**Coral Family** (warm, interaction-focused): #ffeaec (Coral Blush â soft backgrounds), #e1bbc7 (Coral Petal â strokes/borders), #690031 (Deep Coral â text on coral backgrounds), #ff584a (Coral Ember â illustration fills), #710c3a (Coral Dark â hover states). Used in: filter chips, icon illustrations in feature cards, active UI state indicators.

**Violet Family** (cool, informational): #222875 (Asana Violet â product UI frames, feature containers), #cbefff (Sky Ice â informational chip backgrounds), #879fc8 / #6678ac (Blue-300/600 â focus rings and border states). Used in: hero product frame, AI feature callouts, trust/information signals.
...
:root {
  ...
  --font-ghost: 'Ghost', Neue Haas Grotesk Display, Aktiv Grotesk Ex, ui-sans-serif, system-ui, sans-serif;
  --font-twk-lausanne: 'TWK Lausanne', Lausanne (Weltkern), Inter, DM Sans, ui-sans-serif, system-ui, sans-serif;
  --text-caption: 11px;
