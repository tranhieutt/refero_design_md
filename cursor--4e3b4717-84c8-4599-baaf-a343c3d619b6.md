...
| --- | --- | --- | --- |
| Canvas Parchment | `#f7f7f4` | `--color-canvas-parchment` | Page backgrounds, card backgrounds, neutral button backgrounds â provides a soft, warm foundation. |
| Inkwell | `#262510` | `--color-inkwell` | Primary text, strong borders, navigation text â grounds the lighter surfaces with significant contrast. |
| Muted Stone | `#7a7974` | `--color-muted-stone` | Secondary text, subtle borders, icon fills â a mid-tone gray for less prominent information or structural lines. |
| Deep Shadow | `#141414` | `--color-deep-shadow` | Deepest text variant â for maximum contrast on headlines or critical information. |
| Pebble Gray | `#e6e5e0` | `--color-pebble-gray` | Hover states on neutral buttons, subtle card backgrounds â visually lighter than Canvas Parchment, indicating elevation. |
| Onyx Outline | `#f54e00` | `--color-onyx-outline` | Outlined action button borders and link text â a vibrant orange indicating interactive elements without a solid fill. |
| Chartreuse Alert | `#4ade80` | `--color-chartreuse-alert` | supporting accents, small interactive text snippets â a vivid green for positive or noteworthy cues, often within code examples. |
...
| Forest Green Action | `#34785c` | `--color-forest-green-action` | Specific button backgrounds/borders like 'View PR' â a moderate green for distinct, yet secondary actions. |
| Highlight Beige | `#cdcdc9` | `--color-highlight-beige` | Subtle card backgrounds on nested elements, faint border color â a light neutral for separation with low visual weight. |
...
### EB Garamond - EB Garamond â detected in extracted data but not described by AI - `--font-eb-garamond`
- **Weights:** 400, 500
- **Sizes:** 16px
- **Line height:** 1, 1.5
- **Role:** EB Garamond â detected in extracted data but not described by AI
...
:root {
  ...
  --font-cursorgothic: 'CursorGothic', system-ui, ui-sans-serif, system-ui, sans-serif;
  --font-berkeleymono: 'berkeleyMono', monospace, ui-sans-serif, system-ui, sans-serif;
