...
| Pine Border | `#233630` | `--color-pine-border` | Subtle dividers within dark sections â barely distinct from the background, read more as inset lines than separators |
| Slate Ink | `#313942` | `--color-slate-ink` | Default body text, icons, borders on light sections â the near-black that does almost all the work in the light zone |
...
| Parchment | `#f4f7f2` | `--color-parchment` | Page background, section alternates â a barely-warm white that keeps the canvas from feeling sterile |
| Dew | `#e4ebe2` | `--color-dew` | Card borders, image frames, soft dividers on light surfaces |
| Sprout | `#27ae60` | `--color-sprout` | Primary CTA buttons, active badges, success states â the one green that appears in both dark and light zones, making every interactive element identifiable at a glance |
| Sage Whisper | `#defaca` | `--color-sage-whisper` | Accent text within dark hero section â used on word-level highlights inside headlines (e.g. 'Level up' in a contrasting tone), gives the dark hero warmth without switching to orange or gold |
...
| Iris | `#5c51e0` | `--color-iris` | Accent word highlights in light-section headings ('beautiful' in 'Simple products for beautiful designs') â appears word-level only, never as a background |
| Sky Link | `#186bff` | `--color-sky-link` | Inline links and secondary interactive elements in list contexts |
| Tangerine CTA | `#ff5722` | `--color-tangerine-cta` | Article category badges â vivid orange tags on blog/article cards for visual taxonomy |
| Amber Nav | `#f49a40` | `--color-amber-nav` | Sign Up button in the navigation bar â the only warm-hued button; its contrast against the dark nav makes it pop without using the primary green |
| Azure Action | `#2f80ed` | `--color-azure-action` | Icon fill and secondary action states â appears in product UI screenshots and icon illustrations |
| Lavender Tint | `#f1ebff` | `--color-lavender-tint` | Light violet background tint on select feature sections |

## Tokens - Typography

### InterVariable - The sole typeface for the entire system. Display sizes (48â57px) use weight 700 with tight negative tracking (-0.028em to -0.022em), creating dense, confident headlines. Body text (14â16px) uses weight 400 at 1.5â1.6 leading. Weight 600 handles subheadings and UI labels. The five OpenType features (cv03, cv04, cv06, cv09, ss03) distinguish this from stock Inter â alternate letterforms on 'a', 'g', and 'l' give the text a slightly more geometric feel than default Inter. - `--font-intervariable`
...
- **Line height:** 1.1â1.7 for headings; 1.5â1.65 for body; 1.0 for display numerics
...
- **Role:** The sole typeface for the entire system. Display sizes (48â57px) use weight 700 with tight negative tracking (-0.028em to -0.022em), creating dense, confident headlines. Body text (14â16px) uses weight 400 at 1.5â1.6 leading. Weight 600 handles subheadings and UI labels. The five OpenType features (cv03, cv04, cv06, cv09, ss03) distinguish this from stock Inter â alternate letterforms on 'a', 'g', and 'l' give the text a slightly more geometric feel than default Inter.

### Type Scale
| Role | Size | Line Height | Letter Spacing | Token |
| --- | --- | --- | --- | --- |
| caption | 11px | 2.29 | - | `--text-caption` |
...
Background #27ae60, white text, 999px radius (full pill), 13px top/bottom padding, 28px left/right padding. Weight 600, 16px. The same shape and color appears on both dark and light backgrounds â the green reads clearly on either surface. Arrow glyph appended inline (â).
...
White background, #313942 text and border, 17px radius (not a pill â distinct from CTAs), 7px top/bottom padding, 19px left/right padding. The non-pill radius on this variant signals a different interaction tier from the rounded CTAs.
...
#f49a40 background (amber-orange), white text, 999px radius, matching padding to ghost white button. The amber color is unique to this one button site-wide â it's not part of the standard CTA system, deliberately distinguishing account creation from feature CTAs.
...
12â32px border radius on the image container. Drop shadow: rgba(17, 50, 38, 0.14) 14px 17px 40px 0px â a green-tinted shadow that stays on-brand even for elevation. Screenshots are displayed at full card width, never cropped to icons.

### Logo Bar (Social Proof Strip)
**Role:** Trust logos row â Twilio, Toptal, Vodafone, Yelp, etc.
...
### Do
- Use #27ae60 for all primary interactive elements â CTA buttons, active states, success badges â in both dark and light sections. This is the only green allowed as a button fill.
...
- Set display headlines (48â57px) at weight 700 with letter-spacing -0.022em to -0.028em and InterVariable feature-settings '"cv03" on, "cv04" on, "cv06" on, "cv09" on, "ss03" on'.
- Use #f49a40 exclusively for the navigation Sign Up button â no other element in the system uses amber. Do not repurpose this color for section CTAs.
- Apply the green-tinted shadow (rgba(17, 50, 38, 0.14) 14px 17px 40px 0px) only to product screenshot images â never to cards, modals, or buttons.
...
- Use #5c51e0 (Iris) and #defaca (Sage Whisper) only as inline word-level accents inside headlines â never as button fills or background swatches.

### Don't
- Do not mix the amber nav button (#f49a40) into body CTAs or feature sections â it belongs only in the top navigation, where it uniquely signals account creation.
- Do not apply the green-tinted drop shadow to cards or panels â card boundaries are defined by background-color contrast alone (mint vs white vs page gray).
- Do not use Inter without the OpenType feature-settings â 'cv03', 'cv04', 'cv06', 'cv09', 'ss03' are required to match the on-brand letterform variants.
- Do not use positive letter-spacing (tracking) on any text larger than 12px â the +0.118em value is reserved for uppercase micro-labels at 11px only.
- Do not introduce additional font weights beyond 400, 500, 600, 700 â the four-weight scale is sufficient and adding 300 or 800 breaks the typographic register.
- Do not place article category badges in any color other than #ff5722 â violet, green, or blue badges would conflict with the accent-color hierarchy where those hues carry semantic meaning.
- Do not use card shadows or borders on Feature Product Cards â the #edf9f2 mint background against a white or gray page is the only containment mechanism.

## Surfaces

| Level | Name | Value | Purpose |
| --- | --- | --- | --- |
| 1 | Hero Dark | `#0e231c` | Full-bleed hero, dark nav bar, footer â the deepest surface |
...
4. **Article Blog Card**: White card, 12px image radius. Above image: full-width photo. Below: orange badge (#ff5722, 6px radius, white text 11px weight 500, 4px/8px padding). Author row: 24px avatar + name at 14px #313942. Headline 18px weight 600 #313942. 'Read more â' link 14px #313942 weight 400.
...
:root {
  ...
  --font-intervariable: 'InterVariable', Inter (Google Fonts) with feature-settings applied, ui-sans-serif, system-ui, sans-serif;
  --text-caption: 11px;
