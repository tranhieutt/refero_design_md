...
**Refero Style:** https://styles.refero.design/style/5c32375f-6ef1-4345-9418-ebbb7e887343
...
| Cosmic Lilac | `#553f83` | `--color-cosmic-lilac` | Primary CTA buttons â a calm but distinct call to action against the dark backdrop. |
| Supernova | `#a05fcf` | `--color-supernova` | Focus rings, active state borders, UI highlights â a brighter violet for interactive feedback. |
| Nebula Haze | `#bf92ec` | `--color-nebula-haze` | Decorative highlights, accent text. |
| Warp Drive Glow | `#a05fcf` | `--color-warp-drive-glow` | Decorative background effect â adds a powerful, futuristic energy. |
...
| display | 54px | 1.12 | - | `--text-display` |
...
### Primary CTA Button
**Role:** The main call-to-action, like 'Deploy â'.
...
1.  **Hero Section:** Create a full-bleed hero section with the painterly night sky illustration as the background. Add a centered headline 'Ship software peacefully' in 54px `IBM Plex Serif` font, `Starlight` (#f7f7f8) color, with -1.94px letter-spacing. Below it, create a Primary CTA Button with text 'Deploy â' and a Secondary Button with text 'Demo'.

2.  **Primary CTA Button:** Create a button with text 'Deploy â'. Use a `Cosmic Lilac` (#553f83) background, `Starlight` (#f7f7f8) text, 8px border radius, and `12px 24px` padding. The font should be 16px Inter at weight 500.
...
:root {
  ...
  --color-success: #42946;
  --color-error: #d82c20;
  --font-ibm-plex-serif: 'IBM Plex Serif', Georgia, Times New Roman, ui-sans-serif, system-ui, sans-serif;
  --font-inter: 'Inter', system-ui, -apple-system, ui-sans-serif, system-ui, sans-serif;
  --font-inter-tight: 'Inter Tight', Inter, ui-sans-serif, system-ui, sans-serif;
  --font-ui-monospace: 'ui-monospace', Menlo, Monaco, Consolas, ui-sans-serif, system-ui, sans-serif;
  --text-caption: 12px;
  --leading-caption: 1.5;
  --text-body-sm: 14px;
  --leading-body-sm: 1.5;
