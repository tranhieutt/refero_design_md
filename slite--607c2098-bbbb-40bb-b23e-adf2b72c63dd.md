...
> Warm parchment editorial desk â a workspace where knowledge feels handwritten, not enterprise-stamped.

**Theme:** light
**Source:** https://slite.com
**Refero Style:** https://styles.refero.design/style/607c2098-bbbb-40bb-b23e-adf2b72c63dd

Slite's visual language feels like warm parchment under natural light â a cream-toned workspace where knowledge feels approachable rather than clinical. The dominant #f9efe4 background reads as aged paper without feeling retro, grounded by #3f434a ink-dark text that gives editorial weight to every line. The signature move is typographic contradiction: a serif-adjacent custom face (Garnett) at 64px for display, crossed out in black with 'goes to die' while 'thrives' renders in a hand-lettered cursive script in vivid blue â disruption-by-contrast rather than polish. Feature cards sit on near-white #fdfdfd against the warm cream ground, tagged with muted category badges in yellows, greens, and pinks that never compete with the overall softness. Pill buttons in near-black (#2d2f34) against cream create the sharpest contrast point on every screen, anchoring calls to action without resorting to saturated color.

## Tokens - Colors

| Name | Value | Token | Role |
| --- | --- | --- | --- |
| Parchment | `#f9efe4` | `--color-parchment` | Primary page background and hero â the defining warm cream that separates Slite from the cold-white SaaS default |
| Vellum | `#fdf9f4` | `--color-vellum` | Secondary surface, slightly cooler than Parchment â used for card interiors and section alternation |
| Chalk | `#fdfdfd` | `--color-chalk` | Highest-surface level â card backgrounds, modal surfaces, product UI previews |
| Ink | `#3f434a` | `--color-ink` | Primary text across all contexts â body, headings, labels, icons |
...
| Blueprint | `#2e77e5` | `--color-blueprint` | Interactive text links, icon fills, decorative heading accent â the single chromatic voice in body text contexts |
| Electric Sky | `#176be5` | `--color-electric-sky` | SVG stroke accents in illustration work â technical/decorative, not interactive |
| Spectrum Gradient | `#ef91f7` | `--color-spectrum-gradient` | Hero decorative gradient element â appears on accent strokes in illustration, not UI controls |
...
| Sage | `#547358` | `--color-sage` | Category badge text for 'Compliance' tags â muted green that reads as trustworthy without being a semantic success signal |
...
### Garnett - Display and heading font. At 64px weight 700 it carries the hero headline â a custom grotesque with editorial warmth missing from Inter or DM Sans. Weight 500 handles subheadings at 24â36px. The ss14/ss15/ss19 OpenType features are active and likely handle specific glyph alternates that distinguish it from substitutes. - `--font-garnett`
...
- **Line height:** 1.20â1.88
- **OpenType features:** `"ss14", "ss15", "ss19"`
- **Role:** Display and heading font. At 64px weight 700 it carries the hero headline â a custom grotesque with editorial warmth missing from Inter or DM Sans. Weight 500 handles subheadings at 24â36px. The ss14/ss15/ss19 OpenType features are active and likely handle specific glyph alternates that distinguish it from substitutes.

### UniversalSans - UI and body font. Handles all button labels, body copy, nav items, badges, captions. Weight 400 at 14â16px for body, 600 for button labels and emphasized UI text, 700 for numeric callouts. The custom face has more warmth than Inter â slightly wider apertures that match the cream-toned page feel. - `--font-universalsans`
- **Substitute:** Plus Jakarta Sans or Instrument Sans
- **Weights:** 400, 600, 700
- **Sizes:** 10px, 11px, 12px, 14px, 16px, 24px
- **Line height:** 1.00â1.88
- **OpenType features:** `"ss14", "ss15", "ss19"`
- **Role:** UI and body font. Handles all button labels, body copy, nav items, badges, captions. Weight 400 at 14â16px for body, 600 for button labels and emphasized UI text, 700 for numeric callouts. The custom face has more warmth than Inter â slightly wider apertures that match the cream-toned page feel.

### Geist Mono - Monospace face used sparingly for code snippets or technical labels. Letter-spacing at 0.117em (approximately 1.4px at 12px) â wider than default, creating a deliberate typewriter rhythm. - `--font-geist-mono`
...
- **OpenType features:** `"ss14", "ss15", "ss19"`
- **Role:** Monospace face used sparingly for code snippets or technical labels. Letter-spacing at 0.117em (approximately 1.4px at 12px) â wider than default, creating a deliberate typewriter rhythm.
...
### Primary Pill Button
**Role:** Main CTA â 'Start for free', nav primary action
...
**Role:** Secondary CTA â 'Book a demo', secondary page actions
Background transparent, text #2f2f30, border 1px solid #2f2f30, border-radius 50px, padding 8px 24px. UniversalSans 600 at 14px. Sits beside the Primary Pill without competing â same shape language, less visual weight.

### Ghost Text Button
**Role:** Navigation dropdowns â 'Product', 'Solutions', 'Resources'
...
### Rounded Tag Button
**Role:** In-page toggles and selectors â documentation type switchers
Background #fdfdfd, text #3f434a, border 1px solid #3f434a, border-radius 16px, padding 4px 12px. UniversalSans 400 at 12px. The 16px radius sits between pill and rectangle â softer than a badge, not as rounded as a pill.

### Feature Card
**Role:** Enterprise feature grid items â SSO, SCIM, GDPR etc.
...
### Category Badge
**Role:** Feature card label â SECURITY, CUSTOMIZATION, COMPLIANCE, CONTROL
...
Background #fdfdfd, border-radius 12px, prominent layered shadow. Contains a simulated two-panel document UI â sidebar in light gray, content area in white with pink tinted illustration block (#eebacb). Acts as social proof without photography.
...
Background #f9efe4 (matching page ground â no contrast separation from page). Logo + wordmark left, ghost nav links center, 'Sign in' link + 'Book a demo' outlined button + 'Start for free' pill button right. Height 72px. No border-bottom or shadow â floats seamlessly on cream.

### Tooltip
**Role:** Contextual hover labels
Background hsl(0,1%,23%) â #3B3B3B, text hsl(231,1%,100%) â #FDFDFD. Border-radius 8px, padding 4-8px. UniversalSans 400 at 12px. Per CSS design tokens.

## Do's and Don'ts

### Do
- Use #f9efe4 as the default page background â never white or cold gray; the warmth is the entire atmospheric identity
- Apply border-radius 42-50px to all primary and secondary CTA buttons â the pill shape is non-negotiable brand geometry
...
- Render category badges with border-radius 50px, padding 4px 12px, and the specific color pairs: Ochre text on Buttercup (#fbf4d8), Mauve text on Blossom (#fae9f4), Sage text on near-white â never swap pairs
- Keep feature cards at border-radius 12px with the three-layer shadow (rgba(0,0,0,0.01/0.05/0.10)) â this whisper-shadow is the only elevation used
...
- Restrict chromatic color (Blueprint #2e77e5) to interactive text links and decorative heading accents only â never for backgrounds or borders in feature UI

### Don't
- Don't use pure white (#ffffff) as a page background â it reads cold against Slite's warm typographic palette; use #fdfdfd or #fdf9f4 for card surfaces
- Don't introduce saturated color backgrounds for section bands â the warm cream is the only background color; category badges carry all color variety
...
- Don't add shadows beyond the established three-layer formula â heavier shadows break the lightweight elevation philosophy
- Don't use rectangular buttons (border-radius under 12px) for primary actions â all CTAs must be pill or near-pill shaped
- Don't display third-party logos in their brand colors in trust bars â convert all to #3f434a for visual unity
- Don't place category badges in any color combination outside the established four variants (Security/Customization/Compliance/Control) â ad-hoc badge colors fracture the muted palette

## Surfaces

| Level | Name | Value | Purpose |
| --- | --- | --- | --- |
| 0 | Page Ground | `#f9efe4` | Primary page background â the warm cream canvas all content sits on |
...
Slite uses two fonts for distinct jobs with zero overlap: Garnett owns all display and heading sizes, UniversalSans owns all UI and body sizes. The hero headline uses a strikethrough effect on 'goes to die' in Garnett 700 64px, immediately replaced by 'thrives' in a cursive script at the same size in Blueprint blue (#2e77e5) â this editorial contradiction is the brand's most distinctive typographic moment and should be replicated on any hero that benefits from before/after framing. The Geist Mono face appears only for code-adjacent content at 12px with 0.117em letter-spacing â never stretch it beyond that context. OpenType features ss14, ss15, ss19 are active on all three faces â include font-feature-settings in all CSS declarations.
...
:root {
  ...
  --font-garnett: 'Garnett', DM Sans or Fraunces for warmth, though neither replicates Garnett's geometric-editorial balance, ui-sans-serif, system-ui, sans-serif;
  --font-universalsans: 'UniversalSans', Plus Jakarta Sans or Instrument Sans, ui-sans-serif, system-ui, sans-serif;
  --font-geist-mono: 'Geist Mono', Geist Mono (available via Vercel) or JetBrains Mono, ui-sans-serif, system-ui, sans-serif;
  --text-caption: 10px;
  --leading-caption: 16;
  --text-body: 14px;
  --leading-body: 22;
