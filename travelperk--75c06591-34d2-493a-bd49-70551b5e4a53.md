...
> Lime spark on warm parchment â electric CTA green against aged-paper cream, zero shadows, everything rounded at exactly 26px.

**Theme:** light
**Source:** https://travelperk.com
**Refero Style:** https://styles.refero.design/style/75c06591-34d2-493a-bd49-70551b5e4a53

Perk radiates controlled energy â a lime-charged black-and-cream field where electric #beff50 punches through near-black and warm off-white surfaces. The warm off-white (#f5f5eb) hero background reads as aged paper next to the electric lime, making the palette feel tactile rather than digital. A single custom sans, OTSono, does all the work at every scale from 10px UI labels to 200px display glyphs, with tight 0.89-0.90 leading and -0.03em tracking at display sizes making the oversized headlines feel compressed and purposeful. The 26px radius is the system's dominant shape language â applied uniformly to buttons, cards, and image frames â creating rounded-corner consistency that softens an otherwise high-contrast black/lime/cream palette. No shadows, no gradients â surfaces differ only by background value, with #14140f dark cards, #ffffff white cards, and #f5f5eb warm-cream cards all sharing the same 26px radius and no elevation metaphor.

## Tokens - Colors

| Name | Value | Token | Role |
| --- | --- | --- | --- |
| Electric Lime | `#beff50` | `--color-electric-lime` | Primary CTA buttons, active UI chips, icon accent fills â the single chromatic accent that detonates against every surface (near-black, white, and cream alike), creating urgency without red-coded alarm |
| Near Black | `#14140f` | `--color-near-black` | Primary text color, dark card backgrounds, nav borders â a warmed black (not pure #000000) that pairs with cream for a slightly analog feeling |
...
| Warm Cream | `#f5f5eb` | `--color-warm-cream` | Page hero background, section fills â the warm off-white that makes Electric Lime feel naturalistic rather than neon |
...
| Graphite | `#6e6e64` | `--color-graphite` | Body text, supporting labels, card subtext â warm mid-gray that keeps warmth consistent across the achromatic scale |
...
| Signal Blue | `#144fcc` | `--color-signal-blue` | SVG icon fills â brand-category transport icons only |
| Coral Alert | `#eb3131` | `--color-coral-alert` | Status badges â 'Needs Approval' destructive state only |
| Mint Confirm | `#1dc479` | `--color-mint-confirm` | Status badges â 'Confirmed' positive state only |

## Tokens - Typography

### OTSono - The sole typeface across every context â navigation, badges, body, hero â making the design legible entirely through weight and size differentiation rather than font switching. At 80â200px, 0.89 line-height stacks headlines tighter than their cap-height, creating a compressed slab-like mass. The -0.03em tracking at display sizes and +0.10em at small caps/labels span the entire letter-spacing vocabulary of the system. - `--font-otsono`
...
- **Line height:** 0.89â1.50 (display: 0.89â0.90, body: 1.40â1.50, headings: 1.00â1.20)
...
### Primary CTA Button
**Role:** Main conversion action â 'Book a demo', 'Get started'
Background #beff50, text #14140f, border 1px solid #14140f, border-radius 26px, padding 16px 16px. OTSono 16px weight 500. No shadow. The lime fill makes this the loudest element on any surface â used sparingly, max 2 per page section.

### Ghost Button â Dark
**Role:** Secondary actions on light backgrounds
Background transparent, text #14140f, border 1px solid #14140f, border-radius 26px, padding 16px 12pxâ16px. OTSono 16px weight 400. Used for 'Learn more', 'Show all features' on cream/white backgrounds.

### Ghost Button â Light
...
### Lime Accent Card
**Role:** Hero feature callout â 'Real-time visibility' highlight block
Background #beff50, border-radius 26px, no shadow, padding 24-40px. Text in #14140f or #000000. The only card surface using the brand accent â used once per section to designate the primary featured insight.
...
Full-width, background #beff50, text #14140f, OTSono 14px weight 400. Contains inline 'Learn more' underline link and â dismiss icon. The only globally persistent lime surface â reinforces brand color before hero loads.
...
### Do
- Use #beff50 as the exclusive CTA fill â max one lime button and one lime surface element per viewport; never fill two adjacent containers with lime
- Apply 26px border-radius to all cards, buttons, and image frames regardless of content type â the single radius value IS the shape language
- Set display headlines (64px+) at line-height 0.89â1.00 with -0.03em letter-spacing so stacked lines form a compressed typographic mass
- Keep all surfaces on the warm achromatic scale: #14140f / #30302a / #fafaf5 / #f5f5eb / #ffffff â never introduce cool-tinted grays or blue-cast neutrals
- Use #6e6e64 for all supporting body text and card subtext, reserving #14140f for primary labels and headlines only
- Distinguish card hierarchy by background value alone: white (#ffffff) â cream (#fafaf5) â warm cream (#f5f5eb) â dark (#14140f) â no shadows, no borders needed
- OTSono is the only typeface â use size and weight (400 vs 500) as the sole differentiation between body and emphasis

### Don't
- Never use box-shadow or drop-shadow on any card or button â elevation is achieved through background color contrast, not shadow depth
- Never use more than two border-radius values in a layout: 26px for all cards/buttons/images, 8px for badges/chips, 50% for icon-only circles â no other radii
- Never place Electric Lime (#beff50) text on a white background â lime is a background color only; text on lime must be #14140f or #000000
- Never introduce a second typeface â OTSono handles all scales from 10px UI labels to 200px display; switching fonts breaks the single-voice identity
- Never use cool grays (#9ca3af, #6b7280 etc.) â all neutrals must carry the warm undertone of the #f5f5eb â #6e6e64 â #14140f scale
- Never stack two dark cards (#14140f) without a cream or white surface between them â the dark/light alternation defines the page rhythm
- Never apply the 26px radius to inline text links, dividers, or table rows â radius belongs only on bounded box elements

## Surfaces

| Level | Name | Value | Purpose |
| --- | --- | --- | --- |
| 0 | Page Ground | `#f5f5eb` | Hero background and primary page fill â warm cream that makes lime feel organic not neon |
| 1 | Section Surface | `#fafaf5` | Feature cards and alternating section backgrounds â one step brighter than page ground |
| 2 | Elevated Card | `#ffffff` | White cards floating over cream backgrounds â the contrast provides the 'elevation' without shadow |
| 3 | Dark Surface | `#14140f` | Dark feature cards and announcement strip â maximum contrast against all light surfaces |

## Imagery
Product UI screenshots displayed as floating cards over the hero background, cropped at 26px radius to match the card system. A real hand holding a phone (product mockup) anchors the hero center â lifestyle-adjacent but product-focused, not editorial. The second section features a full-bleed video embed (no autoplay) with a 'Watch full video' pill button overlay, using motion documentary-style footage. Illustrations inside feature cards are flat, line-based, geometric â two-color (black line on lime or cream fill), matching the brand palette exactly. Icons throughout the UI are 1.5px stroke weight, monochrome (#14140f or #ffffff depending on surface). No photography outside the hero hand shot and video. Image-to-text ratio is low â the design is text and UI-screenshot dominant.

## Layout
Max-width ~1200px centered on a full-bleed warm cream (#f5f5eb) hero. Hero is full-viewport with centered headline text (OTSono 80-90px) and floating product UI cards arranged in a loose z-pattern around a central phone mockup. Below hero: alternating white and cream bands, each 60px vertical padding. Feature sections use a horizontal scroll card row (4 cards visible, arrow-navigated) rather than a static grid. Tab-switched feature sections (Automate / Control / Support) show 3-column card layouts within a contained max-width box. Logo bar (social proof) runs full-width on cream. Navigation is sticky, top bar, minimal â no mega-menu. Section transitions are seamless (no visual dividers, pure background-color shifts). Overall density is spacious â generous whitespace between the large display type and card elements.
...
4. Announcement banner: Full-width background #beff50. Text 'Focus on your next breakthrough. We'll handle the shadow work.' OTSono 14px weight 400, #14140f. Inline link 'Learn more' underlined, #14140f. â close icon right-aligned, #14140f.
...
- **Linear** - Flat-surface card system with no shadows â elevation via background color contrast only
...
:root {
  ...
  --font-otsono: 'OTSono', Cabinet Grotesk, Geist, or General Sans (rounded terminals, high x-height grotesque), ui-sans-serif, system-ui, sans-serif;
  --text-caption: 10px;
  --leading-caption: 1.5;
  --text-body: 16px;
  --leading-body: 1.5;
