...
The design feels like a functional schematic on a stark white drafting table. Its nearly monochrome palette â #FFFFFF, #FAFAFA, #EAEAEA, #171717 â creates a utility-first atmosphere where user input is the only source of color. Typography is the main architectural element; a custom sans-serif is used everywhere, with tight negative letter-spacing at large sizes creating dense, impactful headlines. The UI is built from simple primitives: solid black CTAs with an 8px radius and subtly bordered white chips, distinguishing primary commands from secondary suggestions.
...
### Button Group â Action Hierarchy
...
:root {
  ...
  --color-ink: #171717;
  --color-onyx: #000000;
  --font-geistsans: 'GeistSans', Inter, ui-sans-serif, system-ui, sans-serif;
  --font-geistmono: 'GeistMono', IBM Plex Mono, ui-sans-serif, system-ui, sans-serif;
  --text-caption: 10px;
  --leading-caption: 1.5;
  --text-body-sm: 14px;
